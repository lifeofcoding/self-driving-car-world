class Tree {
  constructor(center, size, heightCoef = 0.3) {
    ...
    this.heightCoef = heightCoef; // height of the tree relative to the size of the base
  ...
  #generateLevel(point, size) {
    ...
    for (let a = 0; a < Math.PI * 2; a += Math.PI / 16) {
      const kindofRamdom = Math.cos(((a + this.center.x) * size) % 17) ** 2;
      const noisyRadius = rad * lerp(0.5, 1, kindofRamdom);
